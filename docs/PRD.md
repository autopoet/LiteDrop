...
本文档定义 CodeDrop 第一版的产品范围、业务规则、功能需求、接口边界、数据模型、非功能需求、部署限制和验收标准。
...
CodeDrop 是一个无需注册、支持分片上传和断点续传的云端临时文件分享系统。上传完成后生成取件码，接收方可通过取件码下载文件。
...
CodeDrop 不承诺突破物理网络带宽，也不宣称下载速度一定高于网盘。它提供的是：
...
CodeDrop 面向个人开发者、小团队、实验室和面试演示场景，属于单机轻量应用。
...
DATABASE_PATH=/app/data/codedrop.db
