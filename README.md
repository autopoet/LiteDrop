...
LiteDrop（简传）是一个面向个人和小团队的临时文件分享服务。发送方无需注册，通过浏览器分片上传文件并获得六位取件码；接收方输入取件码即可下载。
...
| 上传文件 | 输入取件码 |
|---|---|
| ![LiteDrop 上传工作区](docs/images/litedrop-upload.png) | ![LiteDrop 取件工作区](docs/images/litedrop-pickup.png) |

## 使用方法

### 发送文件

1. 打开首页的“上传”工作区并选择一个不超过 200 MiB 的文件。
2. 选择保存时间和可下载次数，输入管理员提供的上传口令。
3. 点击“开始上传”。上传期间可以暂停，刷新页面后重新选择同一文件也能继续。
4. 等待服务器合并分片，复制生成的六位取件码并发送给接收方。

### 领取文件

1. 打开顶部的“取件”工作区。
2. 输入发送方提供的六位取件码并点击“查询文件”。
3. 核对文件名、大小和失效时间，然后点击“下载文件”。

### 管理文件

1. 打开“管理”工作区，使用 `.env` 中配置的管理员账号和密码登录。
2. 查看正式文件、上传分片和磁盘剩余空间。
3. 可以搜索、删除分享文件，或手动触发过期数据清理。

> 上传口令和管理员密码由部署者配置。文件到期或达到下载次数后将无法继续领取。

## 为什么做这个项目

临时传输文件时，网盘往往要求注册、安装客户端，免费用户还可能受到产品策略限制。LiteDrop 提供一个可以自行部署的简单选择：服务端不区分会员等级，也不主动限速，文件到期后自动清理。

需要特别说明：

- LiteDrop 不会突破云服务器的带宽上限。
